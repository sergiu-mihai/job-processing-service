...
%% @doc Web server for job_processing_service.

-module('job_processing_service_web').
    ...
    try
        case mochiweb_request:get(method ,Req) of
            Method when Method =:= 'GET' ; Method =:= 'HEAD' ->
                case Path of
                    "hello_world" -> mochiweb_request:respond({200, [{"Content-Type", "text/plain"}], "Hello world!\n"}, Req);
            ...
            'POST' ->
                case Path of
                    "job_tasks" ->  
                        ...
                        Job = mochijson2:decode(Body, [{format, map}]),
                        Result = job_processor:parse_job(Job),
                        ResultEncoded = mochijson2:encode(Result),
                        mochiweb_request:respond({200, [{"Content-Type", "application/json"}], ResultEncoded}, Req);
                    "job_script" ->
                        Body = mochiweb_request:recv_body(Req),
                        Job = mochijson2:decode(Body, [{format, map}]),
                        Result = job_processor:script_job(Job),
                        mochiweb_request:respond({200, [{"Content-Type", "text/plain"}], Result}, Req);
